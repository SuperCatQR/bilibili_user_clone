...
- **缓存过期提示**：缓存超过 24 小时不会自动清除，而是打印提示建议手动运行 `update-cache` 强制刷新
...
- **缓存不过期自动清除**：缓存超过 24 小时仅打印提示，不会自动删除，需手动运行 `update-cache` 刷新
...
| `BILIBILI_CLONE_CACHE_TTL_HOURS` | `24` | 缓存过期提示阈值（小时），超过该时间仅打印提示，不会自动清除 |
